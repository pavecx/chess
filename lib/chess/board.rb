...
module Chess
  ...
  class Board
    ...
    # @param position [Position] the position to check
    # @param color [Symbol] the color of the attacking pieces
    # @return [Boolean] true if the square is under attack by the given color
    def square_under_attack?(position, color)
      opponent_pieces.any? do |piece|
        piece.valid_moves(self).include?(position)
      end
    end
    ...
    def setup_pawns
      # White pawns
      8.times do |col|
        @board[6][col] = Chess::Pieces::Pawn.new(:white, Position.new(6, col))
      end

      # Black pawns
      8.times do |col|
        @board[1][col] = Chess::Pieces::Pawn.new(:black, Position.new(1, col))
      end
    end

    def setup_major_pieces
      # White pieces
      @board[7][0] = Chess::Pieces::Rook.new(:white, Position.new(7, 0))
      @board[7][1] = Chess::Pieces::Knight.new(:white, Position.new(7, 1))
      @board[7][2] = Chess::Pieces::Bishop.new(:white, Position.new(7, 2))
      @board[7][3] = Chess::Pieces::Queen.new(:white, Position.new(7, 3))
      @board[7][4] = Chess::Pieces::King.new(:white, Position.new(7, 4))
      @board[7][5] = Chess::Pieces::Bishop.new(:white, Position.new(7, 5))
      @board[7][6] = Chess::Pieces::Knight.new(:white, Position.new(7, 6))
      @board[7][7] = Chess::Pieces::Rook.new(:white, Position.new(7, 7))

      # Black pieces
      @board[0][0] = Chess::Pieces::Rook.new(:black, Position.new(0, 0))
      @board[0][1] = Chess::Pieces::Knight.new(:black, Position.new(0, 1))
      @board[0][2] = Chess::Pieces::Bishop.new(:black, Position.new(0, 2))
      @board[0][3] = Chess::Pieces::Queen.new(:black, Position.new(0, 3))
      @board[0][4] = Chess::Pieces::King.new(:black, Position.new(0, 4))
      @board[0][5] = Chess::Pieces::Bishop.new(:black, Position.new(0, 5))
      @board[0][6] = Chess::Pieces::Knight.new(:black, Position.new(0, 6))
      @board[0][7] = Chess::Pieces::Rook.new(:black, Position.new(0, 7))
    end

    def handle_special_moves(piece, from, to)
      # Handle castling
      if piece.is_a?(Chess::Pieces::King) && (to.col - from.col).abs == 2
        handle_castling(from, to)
      end

      # Handle en passant
      if piece.is_a?(Chess::Pieces::Pawn) && to == @en_passant_target
        handle_en_passant(from, to)
      end

      # Handle pawn promotion
      if piece.is_a?(Chess::Pieces::Pawn) && (to.row == 0 || to.row == 7)
        handle_pawn_promotion(to)
      end

      # Update en passant target
      @en_passant_target = if piece.is_a?(Chess::Pieces::Pawn) && (to.row - from.row).abs == 2
    ...
    def handle_pawn_promotion(position)
      @board[position.row][position.col] = Chess::Pieces::Queen.new(@current_player, position)
    end

    def find_king(color)
      Position.all.find do |pos|
        piece = piece_at(pos)
        piece.is_a?(Chess::Pieces::King) && piece.color == color
      end
    end
